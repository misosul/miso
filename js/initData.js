...
async function initDataSnsList() {
    console.log('isLocal :>> ', isLocal);
    if (isLocal) {
        // 로컬 환경
        const response = await fetch(
            url.origin + "/data/local_snsList.json"
        );
        snsList = await response.json();
        ...
        // 배포 상태에서 GitHub API를 사용(이용자가 적을 때)
            ...
            // 배포 상태에서 Local data를 사용(이용자가 많을 때)
            response = await fetch(
                url.origin + `/${siteConfig.repositoryName}/data/local_snsList.json`
            );
